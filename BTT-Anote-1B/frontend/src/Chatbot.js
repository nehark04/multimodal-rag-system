...
const Chatbot = (props) => {
  ...
  useEffect(() => {
    ...
    setMessages([
      {
        message: "Hello, I am your AI assistant, upload a file to get started.",
        sentTime: "just now",
        direction: "incoming",
      },
    ]);
    ...
    try {
      const response = await fetcher("process-message-pdf-demo", {
        ...
        body: JSON.stringify({
          message: text,
          chat_id: chat_id,
          // document_content: processedText, // Pass the processed text as context for the response
        }),
      });
      ...
      <div className="min-h-[70vh] h-[70vh] mt-2 relative p-4 w-full rounded-2xl">
        <>
          ...
          <div className="flex items-center w-[95%] mx-auto absolute bottom-5 ">
            ...
            <input
              className="w-full rounded-xl bg-gray-900 border-[#9B9B9B] focus:ring-0 focus:border-white text-white placeholder:text-[#9B9B9B]"
              type="text"
